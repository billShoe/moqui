...
moqui.loadStylesheet=function(a,b,c){b||(b="stylesheet");c||(c="text/css");var d=!1;$("head link").each(function(b,c){-1!==c.href.indexOf(a)&&(d=!0)});if(!d){var e=document.createElement("link");e.href=a;e.rel=b;e.type=c;document.head.appendChild(e)}};moqui.retryInlineScript=function(a,b){try{eval(a)}catch(c){a=a.trim(),console.warn("error "+b+" running inline script: "+a.slice(0,30)+"..."),console.warn(c),5>=b&&setTimeout(moqui.retryInlineScript,200,a,b+1)}};
moqui.notifyOpts={delay:1500,offset:{x:20,y:60},placement:{from:"top",align:"right"},z_index:1100,type:"success",animate:{enter:"animated fadeInDown",exit:""}};moqui.notifyOptsInfo={delay:3E3,offset:{x:20,y:60},placement:{from:"top",align:"right"},z_index:1100,type:"info",animate:{enter:"animated fadeInDown",exit:""}};moqui.notifyOptsError={delay:5E3,offset:{x:20,y:60},placement:{from:"top",align:"right"},z_index:1100,type:"danger",animate:{enter:"animated fadeInDown",exit:""}};
moqui.notifyMessages=function(a,b){var c=!1;if(a)if(moqui.isArray(a))for(var d=0;d<a.length;d++)$.notify({message:a[d]},moqui.notifyOptsInfo),moqui.webrootVue.addNotify(a[d],"info"),c=!0;else $.notify({message:a},moqui.notifyOptsInfo),moqui.webrootVue.addNotify(a,"info"),c=!0;if(b)if(moqui.isArray(b))for(d=0;d<b.length;d++)$.notify({message:b[d]},moqui.notifyOptsError),moqui.webrootVue.addNotify(b[d],"danger"),c=!0;else $.notify({message:b},moqui.notifyOptsError),moqui.webrootVue.addNotify(b,"danger"),
c=!0;return c};
moqui.handleAjaxError=function(a,b,c){var d=a.responseText,e;try{e=JSON.parse(d)}catch(h){}console.warn("ajax "+b+" ("+a.status+"), message "+c);var f=!1;e&&moqui.isPlainObject(e)?f=moqui.notifyMessages(e.messages,e.errors):d&&moqui.isString(d)&&d.length&&(f=moqui.notifyMessages(d));401===a.status?moqui.webrootVue?window.location.href=moqui.webrootVue.currentLinkUrl:window.location.reload(!0):0===a.status?0>c.indexOf("abort")&&($.notify({message:"Could not connect to server"},moqui.notifyOptsError),moqui.webrootVue.addNotify("Could not connect to server",
"danger")):f||(a="Error: "+c+" ("+b+")",$.notify({message:a},moqui.notifyOptsError),moqui.webrootVue.addNotify(a,"danger"))};moqui.componentCache=new moqui.LruMap(50);moqui.handleLoadError=function(a,b,c){moqui.webrootVue.loading=0;moqui.handleAjaxError(a,b,c)};
moqui.loadComponent=function(a,b,c){var d,e,f,h;if("string"===typeof a){var g=a.indexOf("?");0<g?(d=a.slice(0,g),f=a.slice(g+1)):d=a}else d=a.path,e=a.extraPath,f=a.search,h=a.bodyParameters;if(a=moqui.componentCache.get(d))console.info("found cached component for path "+d),b(a);else{var k=d,m=".js"===d.slice(-3);m||(k+=".vuet");e&&0<e.length&&(k+="/"+e);f&&0<f.length&&(k+="?"+f);console.info("loadComponent "+k+(c?" id "+c:""));e={type:"GET",url:k,error:moqui.handleLoadError,success:function(a,e,
f){a||b(moqui.NotFound);var h=0<=f.getResponseHeader("Cache-Control").indexOf("max-age");if(moqui.isString(a)&&0<a.length)if(m||"define("===a.slice(0,7)){console.info("loaded JS from "+k+(c?" id "+c:""));var g=eval(a);g.template?(h&&moqui.componentCache.put(d,g),b(g)):(a=(".js"===d.slice(-3)?d.slice(0,-3):d)+".vuet",$.ajax({type:"GET",url:a,error:moqui.handleLoadError,success:function(a){g.template=a;h&&moqui.componentCache.put(d,g);b(g)}}))}else a=a.replace(/<script/g,"\x3cm-script").replace(/<\/script>/g,
"\x3c/m-script\x3e").replace(/<link/g,"\x3cm-stylesheet"),console.info("loaded HTML template from "+k+(c?" id "+c:"")),a={template:"\x3cdiv"+(c&&0<c.length?' id\x3d"'+c+'"':"")+"\x3e"+a+"\x3c/div\x3e"},h&&moqui.componentCache.put(d,a),b(a);else moqui.isPlainObject(a)?a.screenUrl&&0<a.screenUrl.length?this.$root.setUrl(a.screenUrl):a.redirectUrl&&0<a.redirectUrl.length&&window.location.replace(a.redirectUrl):b(moqui.NotFound)}};h&&!$.isEmptyObject(h)&&(e.type="POST",e.data=h);$.ajax(e)}};
moqui.NotFound=Vue.extend({template:'\x3cdiv id\x3d"current-page-root"\x3e\x3ch4\x3eScreen not found at {{this.$root.currentPath}}\x3c/h4\x3e\x3c/div\x3e'});moqui.EmptyComponent=Vue.extend({template:'\x3cdiv id\x3d"current-page-root"\x3e\x3cdiv class\x3d"spinner"\x3e\x3cdiv\x3eLoading…\x3c/div\x3e\x3c/div\x3e\x3c/div\x3e'});
Vue.component("m-link",{props:{href:{type:String,required:!0},loadId:String},template:'\x3ca :href\x3d"linkHref" @click.prevent\x3d"go"\x3e\x3cslot\x3e\x3c/slot\x3e\x3c/a\x3e',methods:{go:function(a){0===a.button&&(this.loadId&&0<this.loadId.length?this.$root.loadContainer(this.loadId,this.href):a.ctrlKey||a.metaKey?window.open(this.linkHref,"_blank"):this.$root.setUrl(this.linkHref))}},computed:{linkHref:function(){return 0===this.href.indexOf(this.$root.basePath)?this.href.replace(this.$root.basePath,
this.$root.linkBasePath):this.href}}});Vue.component("m-script",{props:{src:String,type:{type:String,"default":"text/javascript"}},template:'\x3cdiv :type\x3d"type" style\x3d"display:none;"\x3e\x3cslot\x3e\x3c/slot\x3e\x3c/div\x3e',created:function(){this.src&&0<this.src.length&&moqui.loadScript(this.src)},mounted:function(){var a=this.$el.innerText;a&&0<a.trim().length&&moqui.retryInlineScript(a,1)}});
...
!this.isBodyOpen}}});Vue.component("box-body",{template:'\x3cdiv class\x3d"panel-body"\x3e\x3cslot\x3e\x3c/slot\x3e\x3c/div\x3e'});
Vue.component("container-dialog",{props:{id:{type:String,required:!0},title:String,width:{type:String,"default":"760"},openDialog:{type:Boolean,"default":!1}},data:function(){return{isHidden:!0,dialogStyle:{width:this.width+"px"}}},template:'\x3cdiv :id\x3d"id" class\x3d"modal dynamic-dialog" aria-hidden\x3d"true" style\x3d"display:none;" tabindex\x3d"-1"\x3e\x3cdiv class\x3d"modal-dialog" :style\x3d"dialogStyle"\x3e\x3cdiv class\x3d"modal-content"\x3e\x3cdiv class\x3d"modal-header"\x3e\x3cbutton type\x3d"button" class\x3d"close" data-dismiss\x3d"modal" aria-hidden\x3d"true"\x3e\x26times;\x3c/button\x3e\x3ch4 class\x3d"modal-title"\x3e{{title}}\x3c/h4\x3e\x3c/div\x3e\x3cdiv class\x3d"modal-body"\x3e\x3cslot\x3e\x3c/slot\x3e\x3c/div\x3e\x3c/div\x3e\x3c/div\x3e\x3c/div\x3e',methods:{hide:function(){$(this.$el).modal("hide")}},
mounted:function(){var a=$(this.$el),b=this;a.on("hidden.bs.modal",function(){b.isHidden=!0});a.on("shown.bs.modal",function(){b.isHidden=!1;a.find(":not(.noResetSelect2)\x3eselect:not(.noResetSelect2)").select2({});var c=a.find(".default-focus");c.length?c.focus():a.find("form :input:visible:first").focus()});this.openDialog&&a.modal("show")}});
...
Vue.component("tree-item",{template:'\x3cli :id\x3d"model.id"\x3e\x3ci v-if\x3d"isFolder" @click\x3d"toggle" class\x3d"glyphicon" :class\x3d"{\'glyphicon-chevron-right\':!open, \'glyphicon-chevron-down\':open}"\x3e\x3c/i\x3e\x3cspan v-else style\x3d"width:1em;display:inline-block;"\x3e\x3c/span\x3e \x3cspan @click\x3d"setSelected"\x3e\x3cm-link v-if\x3d"model.a_attr" :href\x3d"model.a_attr.urlText" :load-id\x3d"model.a_attr.loadId" :class\x3d"{\'text-success\':selected}"\x3e{{model.text}}\x3c/m-link\x3e\x3cspan v-if\x3d"!model.a_attr" :class\x3d"{\'text-success\':selected}"\x3e{{model.text}}\x3c/span\x3e\x3c/span\x3e\x3cul v-show\x3d"open" v-if\x3d"hasChildren"\x3e\x3ctree-item v-for\x3d"model in model.children" :key\x3d"model.id" :model\x3d"model" :top\x3d"top"/\x3e\x3c/ul\x3e\x3c/li\x3e',props:{model:Object,
...
(d.is("[name]")&&(b=d.attr("name"),c=d.val()),d.prop("disabled",!0),setTimeout(function(){d.prop("disabled",!1)},3E3));var e=new FormData(this.$el);e.append("moquiSessionToken",this.$root.moquiSessionToken);$.each(this.fields,function(a,b){e.append(a,b)});b&&e.append(b,c);this.$root.loading++;$.ajax({type:this.method,url:this.action,data:e,contentType:!1,processData:!1,headers:{Accept:"application/json"},error:moqui.handleLoadError,success:this.handleResponse})}else if(!this.noValidate){var f=a.find("div.has-error input, div.has-error select, div.has-error textarea").first();
f.length&&(a=f.parents("div.panel-collapse.collapse").last(),a.length?a.hasClass("in")?f.focus():(a=a.prevAll("div.panel-heading").last(),a.length?(b=a.find('a[data-toggle\x3d"collapse"]').first(),b.length?b.click():a.click(),setTimeout(function(){f.focus()},250)):f.focus()):f.focus())}},handleResponse:function(a){this.$root.loading--;var b=!1;a&&moqui.isPlainObject(a)?(b=moqui.notifyMessages(a.messages,a.errors),a.screenUrl&&0<a.screenUrl.length?this.$root.setUrl(a.screenUrl):a.redirectUrl&&0<a.redirectUrl.length&&
(window.location.href=a.redirectUrl)):console.warn("m-form no response or non-JSON response: "+JSON.stringify(a));(a=this.submitHideId)&&0<a.length&&$("#"+a).modal("hide");(a=this.submitReloadId)&&0<a.length&&this.$root.reloadContainer(a);(a=this.submitMessage)&&a.length?(b=eval('"'+a+'"'),$.notify({message:b},moqui.notifyOpts),moqui.webrootVue.addNotify(b,"success")):b||$.notify({message:"Form data saved"},moqui.notifyOpts)},fieldChange:function(a){a=a.delegateTarget;var b=$(a);b.hasClass("input-group")&&
b.children("input").length&&(b=b.children("input").first(),a=b.get(0));var c=!1;if("INPUT"===a.nodeName||"TEXTAREA"===a.nodeName)c="radio"===b.attr("type")||"checkbox"===b.attr("type")?a.checked!==a.defaultChecked:a.value!==a.defaultValue;else if("SELECT"===a.nodeName)if(a.multiple)for(var d=a.options.length,e=0;e<d;e++){var f=a.options[e];if(f.selected!==f.defaultSelected){c=!0;break}}else c=!a.options[a.selectedIndex].defaultSelected;c?(this.fieldsChanged[b.attr("name")]=!0,b.parents(".form-group").children("label").addClass("is-changed"),
...
(d.is("[name]")&&(b=d.attr("name"),c=d.val()),d.prop("disabled",!0),setTimeout(function(){d.prop("disabled",!1)},3E3));var e=a.serializeArray();$.each(this.fields,function(a,b){e.push({name:a,value:b})});for(var f=[],h=[],a="",g=null,k=0;k<e.length;k++){var m=e[k],l=m.name,n=m.value;0===n.trim().length||"moquiSessionToken"===l||"moquiFormName"===l||0<l.indexOf("%5B%5D")||(0<l.indexOf("_op")||0<l.indexOf("_not")||0<l.indexOf("_ic")?f.push(m):(h.push(l),this.bodyParameterNames&&0<=this.bodyParameterNames.indexOf(l)?
(g||(g={}),g[l]=n):(0<a.length&&(a+="\x26"),a+=encodeURIComponent(l)+"\x3d"+encodeURIComponent(n))))}for(k=0;k<f.length;k++)m=f[k],l=m.name.substring(0,m.name.indexOf("_")),0<=h.indexOf(l)&&(0<a.length&&(a+="\x26"),a+=encodeURIComponent(m.name)+"\x3d"+encodeURIComponent(m.value));b&&c&&c.trim().length&&(0<a.length&&(a+="\x26"),a+=encodeURIComponent(b)+"\x3d"+encodeURIComponent(c));b=this.action;b=0<b.indexOf("?")?b+"\x26"+a:b+"?"+a;this.$root.setUrl(b,g)}},clearForm:function(){var a=$(this.$el);a.find(":radio, :checkbox").removeAttr("checked");
...
this.fetchRows()}});
Vue.component("date-time",{props:{id:String,name:{type:String,required:!0},value:String,type:{type:String,"default":"date-time"},size:String,format:String,tooltip:String,form:String,required:String,autoYear:String},template:'\x3cinput v-if\x3d"type\x3d\x3d\'time\'" type\x3d"text" class\x3d"form-control" :pattern\x3d"timePattern" :name\x3d"name" :value\x3d"value" :size\x3d"sizeVal" :data-toggle\x3d"{tooltip:(tooltip\x26\x26tooltip.length\x3e0)}" :title\x3d"tooltip" :form\x3d"form"\x3e\x3cdiv v-else class\x3d"input-group date" :id\x3d"id"\x3e\x3cinput ref\x3d"dateInput" @focus\x3d"focusDate" @blur\x3d"blurDate" type\x3d"text" class\x3d"form-control" :name\x3d"name" :value\x3d"value" :size\x3d"sizeVal" :data-toggle\x3d"{tooltip:(tooltip\x26\x26tooltip.length\x3e0)}" :title\x3d"tooltip" :form\x3d"form" :required\x3d"required \x3d\x3d \'required\' ? true : false"\x3e\x3cspan class\x3d"input-group-addon"\x3e\x3cspan class\x3d"glyphicon glyphicon-calendar"\x3e\x3c/span\x3e\x3c/span\x3e\x3c/div\x3e',methods:{focusDate:function(){if("time"!==
this.type&&"false"!==this.autoYear){var a=$(this.$refs.dateInput),b=a.val();b&&b.length||a.val((new Date).getFullYear())}},blurDate:function(){if("time"!==this.type){var a=$(this.$refs.dateInput),b=a.val();0<b.indexOf("d")?(a.val(""),a.trigger("change")):0<b.indexOf("hh:mm")?(a.val(b.replace("hh:mm","12:00")),a.trigger("change")):0<b.indexOf(":mm")&&(a.val(b.replace(":mm",":00")),a.trigger("change"))}}},computed:{formatVal:function(){var a=this.format;return a&&0<a.length?a:"time"===this.type?"HH:mm":
"date"===this.type?"YYYY-MM-DD":"YYYY-MM-DD HH:mm"},extraFormatsVal:function(){return"time"===this.type?["LT","LTS","HH:mm"]:"date"===this.type?["l","L","YYYY-MM-DD"]:["YYYY-MM-DD HH:mm","YYYY-MM-DD HH:mm:ss","MM/DD/YYYY HH:mm"]},sizeVal:function(){var a=this.size;return a&&0<a.length?a:"time"===this.type?"9":"date"===this.type?"10":"16"},timePattern:function(){return"^(?:(?:([01]?\\d|2[0-3]):)?([0-5]?\\d):)?([0-5]?\\d)$"}},mounted:function(){var a=this.value,b=this.formatVal,c=$(this.$el);"time"!==
this.type&&(c.datetimepicker({toolbarPlacement:"top",showClose:!0,showClear:!0,showTodayButton:!0,useStrict:!0,defaultDate:a&&a.length?moment(a,this.formatVal):null,format:b,extraFormats:this.extraFormatsVal,stepping:5,locale:this.$root.locale}),c.on("dp.change",function(){c.val(c.find("input").first().val());c.trigger("change")}));"YYYY-MM-DD"===b&&c.find("input").inputmask("yyyy-mm-dd",{clearIncomplete:!1,clearMaskOnLostFocus:!0,showMaskOnFocus:!0,showMaskOnHover:!1,removeMaskOnSubmit:!1});"YYYY-MM-DD HH:mm"===
b&&c.find("input").inputmask("yyyy-mm-dd hh:mm",{clearIncomplete:!1,clearMaskOnLostFocus:!0,showMaskOnFocus:!0,showMaskOnHover:!1,removeMaskOnSubmit:!1})}});moqui.dateOffsets=[{id:"0",text:"This"},{id:"-1",text:"Last"},{id:"1",text:"Next"},{id:"-2",text:"-2"},{id:"2",text:"+2"},{id:"-3",text:"-3"},{id:"3",text:"+3"}];
moqui.datePeriods=[{id:"day",text:"Day"},{id:"7d",text:"7 Days"},{id:"30d",text:"30 Days"},{id:"week",text:"Week"},{id:"month",text:"Month"},{id:"quarter",text:"Quarter"},{id:"year",text:"Year"},{id:"7r",text:"+/-7d"},{id:"30r",text:"+/-30d"}];moqui.emptyOpt={id:"",text:" "};
Vue.component("date-period",{props:{name:{type:String,required:!0},id:String,allowEmpty:Boolean,offset:String,period:String,date:String,form:String},template:'\x3cdiv class\x3d"date-period" :id\x3d"id"\x3e\x3cselect ref\x3d"poffset" :name\x3d"name+\'_poffset\'" :form\x3d"form"\x3e\x3c/select\x3e \x3cselect ref\x3d"period" :name\x3d"name+\'_period\'" :form\x3d"form"\x3e\x3c/select\x3e \x3cdate-time :name\x3d"name+\'_pdate\'" :id\x3d"id+\'_pdate\'" :form\x3d"form" type\x3d"date" :value\x3d"date"/\x3e\x3c/div\x3e',mounted:function(){var a=
$(this.$refs.poffset),b=$(this.$refs.period),c=moqui.dateOffsets.slice(),d=moqui.datePeriods.slice();this.allowEmpty&&(c.unshift(moqui.emptyOpt),d.unshift(moqui.emptyOpt));a.select2({data:c});b.select2({data:d});(c=this.offset)&&c.length&&a.val(c).trigger("change");(a=this.period)&&a.length&&b.val(a).trigger("change")}});
Vue.component("drop-down",{props:{options:Array,value:[Array,String],combo:Boolean,allowEmpty:Boolean,multiple:String,optionsUrl:String,serverSearch:{type:Boolean,"default":!1},serverDelay:{type:Number,"default":300},serverMinLength:{type:Number,"default":1},optionsParameters:Object,labelField:String,valueField:String,dependsOn:Object,dependsOptional:Boolean,form:String},data:function(){return{curData:null,s2Opts:null,lastVal:null}},template:'\x3cselect :form\x3d"form"\x3e\x3cslot\x3e\x3c/slot\x3e\x3c/select\x3e',
methods:{processOptionList:function(a,b){var c=[];this.allowEmpty&&(!b||1>=b)&&c.push({id:" ",text:" "});var d=this.labelField;d||(d="label");var e=this.valueField;e||(e="value");$.each(a,function(a,b){var g=b[e];g||(g=" ");c.push({id:g,text:b[d]})});return c},serverData:function(a){var b=!0,c=this.dependsOn,d=this.optionsParameters,e={moquiSessionToken:this.$root.moquiSessionToken},f;for(f in d)d.hasOwnProperty(f)&&(e[f]=d[f]);for(var h in c)c.hasOwnProperty(h)&&((d=$("#"+c[h]).val())&&" "!==d?e[h]=
d:b=!1);a?(e.term=a.term||"",e.pageIndex=(a.page||1)-1):this.serverSearch&&(e.term="",e.pageIndex=0);e.hasAllParms=b;return e},processResponse:function(a,b){if(moqui.isArray(a))return{results:this.processOptionList(a)};b.page=b.page||1;var c=a.pageSize||20;return{results:this.processOptionList(a.options,b.page),pagination:{more:a.count?b.page*c<a.count:!1}}},populateFromUrl:function(a){if(this.optionsUrl&&0!==this.optionsUrl.length)if(a=this.serverData(a),a.hasAllParms||this.dependsOptional){var b=
this;$.ajax({type:"POST",url:this.optionsUrl,data:a,dataType:"json",headers:{Accept:"application/json"},error:moqui.handleAjaxError,success:function(a){if(a=moqui.isArray(a)?a:a.options)b.curData=b.processOptionList(a)}})}else this.curData=null}},mounted:function(){var a=$(this.$el),b=this,c={minimumResultsForSearch:10};this.combo&&(c.tags=!0,c.tokenSeparators=[","," "]);"multiple"===this.multiple&&(c.multiple=!0,c.closeOnSelect=!1,c.width="100%",a.css("min-width","240px"),a.addClass("noResetSelect2"));
this.options&&0<this.options.length&&(c.data=this.options);this.serverSearch&&(this.optionsUrl||console.error("drop-down in form "+this.form+" has no options-url but has server-search\x3dtrue"),c.ajax={url:this.optionsUrl,type:"POST",dataType:"json",delay:this.serverDelay,cache:!0,data:this.serverData,processResults:this.processResponse,error:moqui.handleAjaxError},c.minimumInputLength=this.serverMinLength,c.minimumResultsForSearch=0,c.width="100%",a.css("min-width","200px"),a.addClass("noResetSelect2"));
this.s2Opts=c;a.select2(c);var d=this.value;d&&d.length&&(this.curVal=d);if(this.optionsUrl&&0<this.optionsUrl.length){var a=this.dependsOn,e;for(e in a)if(a.hasOwnProperty(e))$("#"+a[e]).on("change",function(){b.populateFromUrl({term:d})});this.serverSearch?d&&d.length&&moqui.isString(d)&&this.populateFromUrl({term:d}):this.populateFromUrl()}},computed:{curVal:{get:function(){return $(this.$el).select2().val()},set:function(a){$(this.$el).val(a).trigger("change")}}},watch:{value:function(a){this.curVal=
a},options:function(a){this.curData=a},curData:function(a){var b=$(this.$el),c=this,d=b.select2().val();d&&1<d.length&&(this.lastVal=d);b.select2("destroy");b.empty();this.s2Opts.data=a;b.select2(this.s2Opts);setTimeout(function(){var d=c.lastVal;if(!d||2>d.length)d=c.value;if(d){var f=!1,h=moqui.isArray(d);$.each(a,function(a,b){if(h?$.inArray(b.id,d):b.id===d)f=!0});f&&b.val(d)}b.trigger("change")},50)}},destroyed:function(){$(this.$el).off().select2("destroy")}});
...
this.acParameters,h={term:b.val(),moquiSessionToken:this.$root.moquiSessionToken},g;for(g in f)f.hasOwnProperty(g)&&(h[g]=f[g]);for(e in d)d.hasOwnProperty(e)&&(f=$("#"+d[e]).val())&&(h[e]=f);$.ajax({url:this.url,type:"POST",dataType:"json",data:h,error:moqui.handleAjaxError,success:function(d){d=moqui.isArray(d)?d:d.options;for(var e=b.val(),f=0;f<d.length;f++)if(d[f].value==e){a.val(d[f].label);c&&c.html(d[f].label);break}}})}}});
...
Vue.component("subscreens-active",{data:function(){return{activeComponent:moqui.EmptyComponent,pathIndex:-1,pathName:null}},template:'\x3ccomponent :is\x3d"activeComponent"\x3e\x3c/component\x3e',methods:{loadActive:function(){var a=this,b=a.$root,c=a.pathIndex,d=b.currentPathList,e=d[c],f=this.pathName!==e;this.pathName=e;if(!e||0===e.length)return this.activeComponent=moqui.EmptyComponent,!0;e=b.basePath+"/"+d.slice(0,c+1).join("/");if(!f&&moqui.componentCache.containsKey(e))return!1;f={path:e};
c===d.length-1&&(d=b.extraPathList)&&0<d.length&&(f.extraPath=d.join("/"));(d=b.currentSearch)&&0<d.length&&(f.search=d);f.bodyParameters=b.bodyParameters;console.info("subscreens-active loadActive pathIndex "+c+" pathName "+a.pathName+" urlInfo "+JSON.stringify(f));b.loading++;moqui.loadComponent(f,function(c){a.activeComponent=c;b.loading--});return!0}},mounted:function(){this.$root.addSubscreen(this)}});
moqui.webrootVue=new Vue({el:"#apps-root",data:{basePath:"",linkBasePath:"",currentPathList:[],extraPathList:[],activeSubscreens:[],currentParameters:{},bodyParameters:null,navMenuList:[],navHistoryList:[],navPlugins:[],notifyHistoryList:[],lastNavTime:Date.now(),loading:0,activeContainers:{},moquiSessionToken:"",appHost:"",appRootPath:"",userId:"",locale:"en",notificationClient:null},methods:{setUrl:function(a,b){this.bodyParameters=b;$(".modal.in").modal("hide");0===a.indexOf(this.basePath)&&(a=
a.replace(this.basePath,this.linkBasePath));if(this.currentLinkUrl===a&&a!==this.linkBasePath)this.reloadSubscreens();else{var c=a,d=c.indexOf("//");if(0<=d){d=c.indexOf("/",d+1);if(-1===d)return;c=c.slice(d)}c=c.split("?");this.extraPathList=[];this.currentSearch=1<c.length&&0<c[1].length?c[1]:"";this.currentPath=c[0];c=this.currentSearch;if((c=this.currentPath+(0<c.length?"?"+c:""))&&0!==c.length){console.info("current URL changing to "+c);this.lastNavTime=Date.now();this.activeContainers={};var e=
this;$.ajax({type:"GET",url:"/menuData"+c,dataType:"text",error:moqui.handleAjaxError,success:function(a){var b=null;try{b=JSON.parse(a)}catch(c){console.info("Error parson menu list JSON: "+c)}b&&moqui.isArray(b)&&(e.navMenuList=b)}});window.history.pushState(null,this.ScreenTitle,a)}}},setParameters:function(a){a&&(this.$root.currentParameters=$.extend({},this.$root.currentParameters,a));this.$root.reloadSubscreens()},addSubscreen:function(a){a.pathIndex=this.activeSubscreens.length;a.loadActive();
this.activeSubscreens.push(a)},reloadSubscreens:function(){var a=this.currentPathList,b=this.activeSubscreens;if(0===a.length&&0<b.length)b.splice(1),b[0].loadActive();else for(var c=0;c<b.length&&!(c>=a.length);c++)b[c].loadActive()&&b.splice(c+1)},addContainer:function(a,b){this.activeContainers[a]=b},reloadContainer:function(a){var b=this.activeContainers[a];b?b.reload():console.error("Container with ID "+a+" not found, not reloading")},loadContainer:function(a,b){var c=this.activeContainers[a];
c?c.load(b):console.error("Container with ID "+a+" not found, not loading url "+b)},addNavPlugin:function(a){var b=this;moqui.loadComponent(a,function(a){b.navPlugins.push(a)})},addNotify:function(a,b){var c=this.notifyHistoryList.slice(0),d=new Date,e=d.getHours();10>e&&(e="0"+e);var f=d.getMinutes();10>f&&(f="0"+f);d=d.getSeconds();10>d&&(d="0"+d);for(c.unshift({message:a,type:b,time:e+":"+f+":"+d});25<c.length;)c.pop();this.notifyHistoryList=c},switchDarkLight:function(){var a=$("body");a.toggleClass("bg-dark");
a.toggleClass("bg-light");a=a.hasClass("bg-dark")?"bg-dark":"bg-light";$.ajax({type:"POST",url:"/apps/setPreference",error:moqui.handleAjaxError,data:{moquiSessionToken:this.moquiSessionToken,preferenceKey:"OUTER_STYLE",preferenceValue:a}})},showScreenDocDialog:function(a){$("#screen-document-dialog").modal("show");$("#screen-document-dialog-body").load(this.currentPath+"/screenDoc?docIndex\x3d"+a)},stopProp:function(a){a.stopPropagation()},getNavHref:function(a){a||(a=this.navMenuList.length-1);
a=this.navMenuList[a];if(a.extraPathList&&a.extraPathList.length){var b=a.path+"/"+a.extraPathList.join("/"),c=a.pathWithParams.indexOf("?");0<c&&(b+=a.pathWithParams.slice(c));return b}return a.pathWithParams}},watch:{navMenuList:function(a){if(0<a.length){var b=a[a.length-1];a=1<a.length?a[a.length-2]:null;b.extraPathList&&(this.extraPathList=b.extraPathList);var c=this.basePath.split("/").length,c=b.path.split("/").slice(c);console.info("nav updated fullPath "+JSON.stringify(c)+" currentPathList "+
JSON.stringify(this.currentPathList));this.currentPathList=c;this.reloadSubscreens();a=(a?a.title+" - ":"")+b.title;var c=b.pathWithParams,d=c.indexOf("?");if(0<d){for(var e=c.substring(d+1).split("\x26"),c=c.substring(0,d),d="",f=0;f<e.length;f++){var h=e[f];if(0!==h.indexOf("pageIndex\x3d")){var c=-1===c.indexOf("?")?c+"?":c+"\x26",c=c+h,g=h.indexOf("\x3d");if(0<g){var k=h.substring(0,g);0<k.indexOf("_op")||0<k.indexOf("_not")||0<k.indexOf("_ic")||"moquiSessionToken"===k||(0<d.length&&(d+=", "),
d+=decodeURIComponent(h.substring(g+1)))}}}0<d.length&&(70<d.length&&(d=d.substring(0,70)+"..."),a=a+" ("+d+")")}e=this.navHistoryList;for(d=0;d<e.length;)e[d].pathWithParams===c?e.splice(d,1):d++;for(e.unshift({title:a,pathWithParams:c,image:b.image,imageType:b.imageType});25<e.length;)e.pop();document.title=a}},currentPathList:function(a){(a=a[a.length-1])&&$(this.$el).removeClass().addClass(a)}},computed:{currentPath:{get:function(){var a=this.currentPathList,b=this.extraPathList;return this.basePath+
...
"/"+b.join("/"):"")},currentSearch:{get:function(){return moqui.objToSearch(this.currentParameters)},set:function(a){this.currentParameters=moqui.searchToObj(a)}},currentLinkUrl:function(){var a=this.currentSearch;return this.currentLinkPath+(0<a.length?"?"+a:"")},ScreenTitle:function(){return 0<this.navMenuList.length?this.navMenuList[this.navMenuList.length-1].title:""}},created:function(){this.moquiSessionToken=$("#confMoquiSessionToken").val();this.appHost=$("#confAppHost").val();this.appRootPath=
$("#confAppRootPath").val();this.basePath=$("#confBasePath").val();this.linkBasePath=$("#confLinkBasePath").val();this.userId=$("#confUserId").val();this.locale=$("#confLocale").val();moqui.localeMap[this.locale]&&(this.locale=moqui.localeMap[this.locale]);var a=this;$(".confNavPluginUrl").each(function(b,c){a.addNavPlugin($(c).val())});this.notificationClient=new moqui.NotificationClient(("https:"===location.protocol?"wss://":"ws://")+this.appHost+this.appRootPath+"/notws")},mounted:function(){var a=
$(this.$el);a.find('.navbar [data-toggle\x3d"tooltip"]').tooltip({placement:"bottom",trigger:"hover"});a.find("#history-menu-link").tooltip({placement:"bottom",trigger:"hover"});a.find("#notify-history-menu-link").tooltip({placement:"bottom",trigger:"hover"});a.find("#document-menu-link").tooltip({placement:"bottom",trigger:"hover"});this.setUrl(window.location.pathname+window.location.search);this.notificationClient.registerListener("ALL");$("#screen-document-dialog").on("hidden.bs.modal",function(){var a=
$("#screen-document-dialog-body");a.empty();a.append('\x3cdiv class\x3d"spinner"\x3e\x3cdiv\x3eLoading…\x3c/div\x3e\x3c/div\x3e')})}});window.addEventListener("popstate",function(){moqui.webrootVue.setUrl(window.location.pathname+window.location.search)});
